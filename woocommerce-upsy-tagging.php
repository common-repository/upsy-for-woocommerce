...
class WC_upsy_Tagging
{
	...
	const UPSY_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/triggers/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';
...
e("<?php echo esc_url($upsyjsurl); ?>", f, document.body)
	...
	public function upsy_plugin_activation() 
	{
		$webhook_url = self::UPSY_SLACK_WEBHOOK_URL;
		$payload = $this->upsy_create_payload();
		$this->upsy_send_post_request($webhook_url, $payload);
	}

	private function upsy_create_payload()
	{
		$upsy_plugin_version = self::VERSION;
		return array(
			'pluginVersion' => 'Plugin Version: ' . $upsy_plugin_version,
			'adminEmail' => 'Admin Email: ' . get_option('admin_email'),
			'siteUrl' => 'Website Url: ' . get_option('siteurl'),
			'shopName' => 'Shop Name: ' . get_bloginfo('name')
		);
	}

	private function upsy_send_post_request($url, $payload)
	{
